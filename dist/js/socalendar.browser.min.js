"use strict";(()=>{var b=class w{constructor(a={}){this.date=new Date;this.today=new Date;this.referenceDate=new Date(1970,0,4);this.year=new Date().getFullYear();this.month=new Date().getMonth();this.day=new Date().getDay();this.selector=".date-picker";this.dateRestrictions=/[0-9\/-]/g;this.applyMask=a=>{let e=document.getElementById("soCalendar-date-confirm"),t="00/00/0000",n=a.target;if(!(n instanceof HTMLInputElement))return;let s=n.value,l=s.replace(/[^a-zA-Z0-9]/g,"");if(s.trim()==="")return;let c="",r=n.selectionStart||0,d=0;for(let o=0;o<t.length&&!(d>=l.length);o++){let h=l[d];if(t[o]!=="0"&&t[o]!=="A"){c+=t[o];continue}if(t[o]==="0"&&!/\d/.test(h)||t[o]==="A"&&!/[a-zA-Z]/.test(h))break;c+=l[d++];let v=t[o+1];v&&v!=="0"&&v!=="A"&&(c+=v,o++)}n.value=c;let u=c[r-1];if(t[r]!="0"&&t[r]!="A"&&r++,c.length>r)for(;!/\d/.test(u)&&!/[a-zA-Z]/.test(u);)u=c[r-1],r++;n.setSelectionRange(r,r),e.disabled=c.length!==t.length};let e={...w.DEFAULTS,...a};Object.assign(this,e),this.generateWeekDays();let t=document.createElement("dialog");t.id="soCalendar",t.classList.add("socalendar"),t.dataset.socalendarView="date",t.innerHTML=` 
    ...
    `,this.dialog.querySelectorAll("#soCalendar button.soCalendar-select-month").forEach(e=>{e.addEventListener("click",t=>{let n=t.currentTarget;if(n instanceof HTMLButtonElement){console.log("buttn ok");let s=Number(n.dataset.month);console.log(s),s&&(this.month=s,this.updateMonthLabel(),this.generateDatePicker())}})})}generateDatePicker(){this.dialog.dataset.socalendarView="date",this.content.classList.remove("next","prev"),this.toggleElements(["soCalendar-back","soCalendar-edit"],!0),this.toggleElements(["soCalendar-footer","soCalendar-prev-year","soCalendar-prev-month","soCalendar-next-month","soCalendar-next-year"],!1);let a=this.today.getFullYear(),e=this.today.getMonth(),t=this.today.getDate(),n=[this.contentPrev,this.contentCurrent,this.contentNext];for(let l=-1;l<=1;l++){let c=new Date(this.year,this.month+l,1),r=c.getFullYear(),d=c.getMonth(),u=new Date(r,d,1),o=this.daysInMonth(r,d),h=u.getDay();this.weekStartsOnMonday&&(h=(h+6)%7);let v=new Date(r,d-1,1),D=v.getMonth(),f=v.getFullYear(),C=this.daysInMonth(f,D),E=42,M=[];for(let g=0;g<E;g++){let m,i=d,p=r,y=!1;if(g<h)m=C-h+g+1,i=D,p=f,y=!0;else if(g>=h+o){let L=new Date(r,d+1,1);m=g-(h+o)+1,i=L.getMonth(),p=L.getFullYear(),y=!0}else m=g-h+1;let x=new Date(p,i,m),k=!y&&m===t&&i===e&&p===a,B=this.date.getTime()===x.getTime(),T=this.minDate&&new Date(p,i,m)<this.minDate||this.maxDate&&new Date(p,i,m)>this.maxDate,Y=this.minDate instanceof Date&&new Date(p,i,m).getTime()===this.minDate.getTime(),H=this.maxDate instanceof Date&&new Date(p,i,m).getTime()===this.maxDate.getTime();M.push({day:m,month:i,year:p,outside:y,isToday:k,isCurrent:B,isDisabled:T,isMinDate:Y,isMaxDate:H})}n[l+1].innerHTML=`
        <table class="socalendar-date-picker" cellpadding="0" cellspacing="0">
          ...
          ${[0,1,2,3,4,5].map(g=>`<tr>
              ${[0,1,2,3,4,5,6].map(m=>{let i=M[g*7+m],p=`${i.day} ${this.getMonthName(i.month,"long")} ${i.year}`,y=["socalendar-button","soCalendar-select-date",i.outside?"is-outside-month":"",i.isToday?"is-today":"",i.isCurrent?"is-current":"",i.isMinDate?"is-min-date":"",i.isMaxDate?"is-max-date":""].filter(Boolean).join(" ");return`
                  <td>
                    <button
                      aria-label="${p}"
                      type="button"
      ...
      `}this.updateDate(),this.dialog.querySelectorAll("#soCalendar button.soCalendar-select-date").forEach(l=>{l.addEventListener("click",c=>{let r=c.currentTarget;if(r instanceof HTMLButtonElement){let d=r.dataset.year,u=r.dataset.month,o=r.dataset.day;d&&u&&o&&this.setDate(new Date(Number(d),Number(u),Number(o)))}})})}};globalThis.SoCalendar=b;})();
